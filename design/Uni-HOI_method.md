...
- Fusion is HOI-primary with shared CoInteract-style DiT blocks: HOI and RGB tokens share the attention/MLP backbone, while patch/projection layers and stream-specific AdaLN scale/shift modulation distinguish modalities. RGB DiT hidden tokens form an auxiliary stream inside each block, then guide HOI tokens through zero-init cross-stream adapters. `hoi_to_rgb_scale` can be enabled for symmetric warmup/ablations, but the retained prediction head and supervised losses remain HOI-side.
