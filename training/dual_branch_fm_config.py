...
def load_dual_branch_fm_config(
    ...
    train_section["dataloader_num_workers"] = train.get("dataloader_num_workers", "auto")
    train_section["dataloader_pin_memory"] = bool(train.get("dataloader_pin_memory", True))
    train_section["dataloader_persistent_workers"] = bool(train.get("dataloader_persistent_workers", True))
    train_section["dataloader_prefetch_factor"] = int(train.get("dataloader_prefetch_factor", 2))
    train_section["batch_sampler_mode"] = str(train.get("batch_sampler_mode", "global_clip_shuffle"))
    train_section["sequence_batch_interleave_window"] = int(train.get("sequence_batch_interleave_window", 0))
    train_section["sequence_batch_burst"] = int(train.get("sequence_batch_burst", 1))
    train_section["wan_pad_to_compatible_frames"] = bool(train.get("wan_pad_to_compatible_frames", True))
